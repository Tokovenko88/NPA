...
### НЕПРЕРЕКАЕМОЕ ПРАВИЛО: запрет удаления исторических данных
1. **Никогда не удаляйте старые редакции.** Даже если элемент был удалён (`delete`) или заменён новой редакцией (`new_redaction`), старая ревизия с её полным `body` остаётся в JSON как историческая запись.
2. **«Удалить» = закрыть ревизию.** Установите `valid_to` = (`valid_from` нового события - 1 день) и при необходимости добавьте `not_valid` = ID изменяющего НПА. Не стирайте `mod_type`, `modified_by_id` и `body` у закрытых ревизий.
3. **Полная история документа.** JSON хранит историю редактирования документа полностью: по любой дате можно восстановить актуальное состояние документа, перебирая ревизии по `valid_from`/`valid_to`.
4. **Запрещённые операции:**
   - `del item['revisions']`
   - `item['revisions'] = []` (кроме плейсхолдеров перед их слиянием)
   - `rev.pop('mod_type', None)` у закрытых ревизий
   - `rev.pop('modified_by_id', None)` у закрытых ревизий
   - `rev.pop('body', None)` у любых ревизий
   - удаление дочерних элементов из `item_children` без переноса их состояния в новый элемент
5. **Корректный «delete»:** закрытая ревизия должна содержать:
   - `valid_to` = дата предыдущего дня
   - `not_valid` = ID НПА, утратившего силу
   - `mod_type` = "delete" (сохраняется!)
   - `modified_by_id` = ID НПА (сохраняется!)
   - `body` = содержимое на момент удаления (сохраняется!)
...
## 3.5 ЗАПРЕТ: Номера структурных элементов в body paragraphs

**КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО** включать номер структурного элемента (`item_number`) в текст `html_text` абзацев (`paragraph`) для элементов типов `part`, `point`, `subpoint`.

- Номер элемента хранится ТОЛЬКО в поле `item_number` на уровне самого элемента.
- В `body[].html_text` абзацы должны содержать ТОЛЬКО текст абзаца без ведущего номера.
- Пример **НЕВЕРНО**: `"html_text": "<p>1. К отдельным категориям граждан...</p>"` у элемента `part` с `item_number: "1"`.
- Пример **ВЕРНО**: `"html_text": "<p>К отдельным категориям граждан...</p>"` у того же элемента.
- Нумерация добавляется только при отображении/рендеринге документа, но НИКОГДА не сохраняется в `html_text` внутри JSON.
